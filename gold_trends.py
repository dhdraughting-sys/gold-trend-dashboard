...
def write_pwa_assets(out_dir):
    """Write manifest.json + a couple of icon sizes next to the dashboard so
    it can be "Added to Home Screen" on a phone and open like an app, with
    no browser address bar. Icon generation needs Pillow; if it's not
    installed, we just skip the icons (the manifest/meta tags alone still
    make Chrome offer an install prompt on Android, just with a generic
    icon) rather than failing the whole run over a cosmetic touch."""
    import os

    manifest = {
        "name": "Gold Trend Dashboard",
        "short_name": "Gold Trends",
    ...
    try:
        ...
        print("  [warn] Pillow not installed - skipping app icon (dashboard still works). "
    ...
    for size in (192, 512):
        ...
        pad = round(size * 0.14)
        draw.rounded_rectangle([pad, pad, size - pad, size - pad], radius=round(size * 0.22), fill="#1a1a19")

        # A simple upward "stock trend" line with a clean triangular arrowhead,
        # computed with real vector math so the head always points along the
        # line instead of being eyeballed into a mess.
        gold = "#eda100"
        w = size - pad * 2
        p0 = (pad + w * 0.16, pad + w * 0.66)   # trend start, bottom-left
        p1 = (pad + w * 0.44, pad + w * 0.42)   # mid kink
        p2 = (pad + w * 0.82, pad + w * 0.20)   # arrow tip, top-right
        line_w = max(3, round(size * 0.05))

        draw.line([p0, p1], fill=gold, width=line_w, joint="curve")

        # Arrowhead: a filled triangle whose tip is p2 and whose base sits
        # back along the p1->p2 direction, centered on the line.
        dx, dy = p2[0] - p1[0], p2[1] - p1[1]
        length = math.hypot(dx, dy) or 1
        ux, uy = dx / length, dy / length      # unit vector along the line
        perp = (-uy, ux)                       # perpendicular unit vector
        head_len = w * 0.16
        head_half_w = w * 0.09
        base_center = (p2[0] - ux * head_len, p2[1] - uy * head_len)
        base_a = (base_center[0] + perp[0] * head_half_w, base_center[1] + perp[1] * head_half_w)
        base_b = (base_center[0] - perp[0] * head_half_w, base_center[1] - perp[1] * head_half_w)
        # draw the shaft up to the arrowhead's base so there's no gap/overlap
        draw.line([p1, base_center], fill=gold, width=line_w, joint="curve")
        draw.polygon([p2, base_a, base_b], fill=gold)
...
<title>Gold Trends</title>
<link rel="manifest" href="manifest.json">
<link rel="icon" href="icon-192.png">
<link rel="apple-touch-icon" href="icon-192.png">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="apple-mobile-web-app-title" content="Gold Trends">
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero .hero-label { font-size: 13px; color: var(--text-secondary); font-weight: 600; letter-spacing: .02em; }
  .hero .hero-price { font-size: 56px; font-weight: 700; line-height: 1.05; margin: 6px 0 4px; letter-spacing: -0.01em; }
  .hero .hero-delta { font-size: 16px; font-weight: 600; }
  .hero .hero-delta.good { color: var(--good); }
  .hero .hero-delta.critical { color: var(--critical); }
  .hero .hero-sub { font-size: 13px; color: var(--text-muted); margin-top: 6px; }

  .scroll-strip {
    display: flex; gap: 8px; overflow-x: auto; padding: 16px 2px 20px;
    scroll-snap-type: x proximity; -webkit-overflow-scrolling: touch;
  }
  .scroll-strip::-webkit-scrollbar { display: none; }
  .scroll-chip {
    scroll-snap-align: start; flex: 0 0 auto; min-width: 74px; text-align: center;
    background: var(--surface-1); border: 1px solid var(--border); border-radius: 12px; padding: 10px 8px;
  }
  .scroll-chip .chip-label { font-size: 10.5px; color: var(--text-muted); text-transform: uppercase; letter-spacing: .03em; }
  .scroll-chip .chip-value { font-size: 15px; font-weight: 700; margin-top: 5px; }
  .scroll-chip .chip-value.good { color: var(--good); }
  .scroll-chip .chip-value.critical { color: var(--critical); }
...
function render() {
  ...
  const heroTone = toneClass(pc["1D"]);
  const heroHtml = `
    <div class="hero">
      <div class="hero-label">Gold · ${gold.source_name}</div>
      <div class="hero-price">$${fmt(gold.last_price, 2)}</div>
      <div class="hero-delta ${heroTone}">${arrow(pc["1D"])} ${fmtPct(pc["1D"])} today</div>
      <div class="hero-sub">${TREND_LABEL[gold.trend]}${signal ? ` · ${signal.label_text}` : ""}</div>
    </div>`;

  const stripChips = [
    { label: "1D", value: pc["1D"] }, { label: "1W", value: pc["1W"] },
    { label: "1M", value: pc["1M"] }, { label: "3M", value: pc["3M"] },
    { label: "YTD", value: pc["YTD"] }, { label: "1Y", value: pc["1Y"] },
  ];
  const stripHtml = stripChips.map((c) => `
    <div class="scroll-chip">
      <div class="chip-label">${c.label}</div>
      <div class="chip-value ${toneClass(c.value)}">${fmtPct(c.value)}</div>
    </div>`).join("");
  ...
  content.innerHTML = `
    ${heroHtml}
    <div class="scroll-strip">${stripHtml}</div>
    ${signalHtml}
...
def main():
    ...
    write_pwa_assets(out_dir or ".")
    print(f"\nWrote {out}")
    print("Tip: open the dashboard on your phone and use \"Add to Home Screen\" "
          "(Safari share menu, or Chrome's install icon) to get an app-like icon.")
